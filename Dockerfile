...
CMD /bin/bash -c "envsubst '\$PORT' < /etc/nginx/conf.d/default.conf.template > /etc/nginx/conf.d/default.conf && sed -i 's|/usr/share/nginx/html|/usr/src/app/build|g' /etc/nginx/conf.d/default.conf && nginx -g 'daemon off;'"
